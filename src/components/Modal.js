...
const Modal = ({ setIsShowModal, content, name, handleSubmit, queries, arrMinMax, defaultText }) => {
    ...
    const hangdleClickTrack = (e, value) => {
        const stackEl = document.getElementById('track')
        const stackReact = stackEl.getBoundingClientRect()
        let percent = value ? value : Math.round((e.clientX - stackReact.left) * 100 / stackReact.width, 0)
        ...
        <div onClick={() => { setIsShowModal(false) }}
            ...
            <div onClick={(e) => {
                ...
                <div className='h-[45px] flex items-center px-4 border-b border-gray-200'>
                    <span className='cursor-pointer hover:text-orange-500' onClick={(e) => {
                        e.stopPropagation()
                ...
                {(name === 'price' || name === 'area') && <div className='p-10 py-20 '>
                    <div className='flex flex-col items-center justify-center relative'>
                        <div className='z-30 absolute top-[-48px] font-bold text-xl text-orange-500'>
                        ...
                        <div onClick={hangdleClickTrack} id='track' className='slider-track h-[5px] bg-gray-300 absolute top-0 bottom-0 w-full rounded-full'></div>
                        <div onClick={hangdleClickTrack} id='track-active' className='slider-track-active h-[5px] bg-orange-500 absolute  top-0 bottom-0 rounded-full'></div>
                        <input
                            max='100'
                            min='0'
                            step='1'
                        ...
                        />
                        <div className='absolute z-30 top-3 left-0 right-0 flex justify-between items-center'>
                            <span className='px-2'
                            // onClick={(e) => {
                            //     e.stopPropagation()
                            //     hangdleClickTrack(e, 0)
                            // }}
                            >
                                0
                            </span>
                ...
                {(name === 'price' || name === 'area') && <button type='button'
